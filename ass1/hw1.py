...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.zeros([X.get_shape().as_list()[1], layersize]))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    :return: The following variables should be returned in the following order.
        ...
        batch_loss: The average cross-entropy loss of the batch
    """
    w1 = tf.Variable(tf.truncated_normal([X.get_shape().as_list()[1], hiddensize]))
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    initial1 = tf.truncated_normal([filter_shape[0], filter_shape[1], 1, convlayer_sizes[0]], stddev=0.1)
    w1 = tf.Variable(initial1)
    b1 = tf.Variable(tf.constant(0.1, shape=[convlayer_sizes[0]]))
    conv1 = tf.nn.relu(tf.nn.conv2d(X, w1, strides=[1, 1, 1, 1], padding="SAME") + b1)

    initial2 = tf.truncated_normal([filter_shape[0], filter_shape[1], convlayer_sizes[0], convlayer_sizes[1]], stddev=0.1)
    w2 = tf.Variable(initial2)
    b2 = tf.Variable(tf.constant(0.1, shape=[convlayer_sizes[1]]))
    conv2 = tf.nn.relu(tf.nn.conv2d(conv1, w2, strides=[1, 1, 1, 1], padding="SAME") + b2)

    new_shape = 28 * 28 * convlayer_sizes[1]
    X_flat = tf.reshape(conv2, [-1, new_shape])
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(X_flat, Y, layersize=outputsize)
